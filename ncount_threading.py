...
class Motor_bot(object):
    def __init__(self, vertiq, KP, KI, KD, target, motorOff, motorDir):
        global Motor_bot
        self.velocity = 0
        self.vertiq = vertiq
        self.motorOffset = motorOff
        self.PIDbot = PIDbot(KP, KI, KD, target * motorDir, motorOff,motorDir)
...
class Motor_top(object):
    def __init__(self, vertiq, KP, KI, KD, target, motorOff, motorDir):
        global Motor_top
        self.velocity = 0
        self.vertiq = vertiq
        self.motorOffset = motorOff 
        self.PIDtop = PIDtop(KP, KI, KD, target * motorDir, motorOff,motorDir)
...
def find_phase(phase):
    while(ncounter):
        if phase % math.pi == 0:
            n = phase/math.pi
            return n
        
# class phaseCounter(threading.Thread):
...
#         self.alive = True
#         while self.alive:
#             if self.phase % math.pi == 0:
#                 self.n = self.phase/math.pi
...
motor1 = Motor_bot(vertiq1, P, I, D, targetSpeed, motorOff1, motorDir1)
motor2 = Motor_top(vertiq2, P, I, D, targetSpeed, motorOff2, motorDir2)
...
# phasemot1 = threading.Thread(target=find_phase, args=[motor1.vertiq.get("multi_turn_angle_control", "ctrl_angle")])
# phasemot2 = threading.Thread(target=find_phase, args=[motor2.vertiq.get("multi_turn_angle_control", "ctrl_angle")])
# phasemot1.start();
# phasemot2.start();

with concurrent.futures.ThreadPoolExecutor() as executor:
    f1 = executor.submit(find_phase, motor1.vertiq.get("multi_turn_angle_control", "ctrl_angle"))
    f2 = executor.submit(find_phase, motor2.vertiq.get("multi_turn_angle_control", "ctrl_angle"))
    n1 = f1.result()
    n2 = f2.result()
    print("n1=",n1," n2=",n2)

# phasemotTOP = phaseCounter(motor1.vertiq.get("multi_turn_angle_control", "ctrl_angle"))
# phasemotBOT = phaseCounter(motor2.vertiq.get("multi_turn_angle_control", "ctrl_angle"))

## start the count for n
# phasemotTOP.start()
# phasemotBOT.start()
...
#     motor.vertiq.set("multi_turn_angle_control", "trajectory_duration", 1)
motor1.vertiq.set("multi_turn_angle_control", "trajectory_angular_displacement", motor1.motorOffset)
motor1.vertiq.set("multi_turn_angle_control", "trajectory_duration", 1)
motor2.vertiq.set("multi_turn_angle_control", "trajectory_angular_displacement", motor2.motorOffset)
motor2.vertiq.set("multi_turn_angle_control", "trajectory_duration", 1)
...
motor1.PIDbot.startTime = startTime
motor1.PIDbot.prevTime = startTime
motor2.PIDtop.startTime = startTime
motor2.PIDtop.prevTime = startTime
...
while pid:
    ...
    if obsDisplacement1 is not None:
        ...
        velocity1 = motor1.PIDbot.compute(obsDisplacement1)
        motor1.vertiq.set("multi_turn_angle_control", "ctrl_velocity", targetSpeed + velocity1)
    ...
    if obsDisplacement2 is not None:
        ...
        velocity2 = motor2.PIDtop.compute(obsDisplacement2)
        motor2.vertiq.set("multi_turn_angle_control", "ctrl_velocity", motorDir2*(targetSpeed + velocity2))
